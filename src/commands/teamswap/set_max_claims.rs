...
pub async fn set_max_claims(
    ctx: Context<'_>,
    #[description = "Set the maximum amount of anime a team can claim"] 
    max_claims: u64,
) -> Result<(), Error> {
    ...
    if max_claims < 0 {
        ctx.send(CreateReply::default().content("trying to set invalid max claims").ephemeral(true)).await?;
        return Ok(())
    }
    env::set_var("ALLOWED_CLAIMS", format!("{}", max_claims));
    let message = format!("Set phase to: {}", max_claims);
    ctx.send(CreateReply::default().content(message).ephemeral(true)).await?;
    Ok(())
}